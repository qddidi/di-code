...
响应通过 `id` 关联；流事件是 `kind: "event"`，运行事件必须携带 `sessionId` 与 `runId`，通过 `requestId` 关联 operation，并可带单调 `sequence`。新的多会话客户端应为所有 session-scoped 请求显式携带 `sessionId`，`steer`、`cancel`、`get_operation`、审批和 interaction 还应携带目标 `runId`；为兼容旧版单 Session 客户端，省略的归属字段可在当前 active session 上补齐，但显式错误的 session 归属仍会被拒绝。
