...
graph LR
  ...
  Agent --> Runtime["@di-code/plugin-runtime<br/>Context / Fiber / Registry"]
  Runtime --> Loader["@di-code/plugin-loader<br/>Composition / manifest"]
  Loader --> SDK["@di-code/plugin-sdk<br/>第三方公开入口"]
  Loader --> Builtins["@di-code/builtins<br/>内建 namespace entries"]
  Builtins --> Coding["@di-code/coding-agent<br/>产品 bootstrap / CLI"]
...
- `plugin-runtime` 是生命周期和服务所有权边界；`plugin-loader` 是 namespace package、manifest 与 Composition 边界；外部插件只能通过 `plugin-sdk` 根入口引用这些公开契约。
- `builtins` 将 Provider、工具、Session、CLI mode、TUI 和 RPC 作为独立 namespace entry 提供；`coding-agent` 只负责产品 bootstrap、默认 composition、MCP 与用户可见 CLI。
...
packages/
  ...
  plugin-runtime/ Context、Fiber、Service、EventBus、capability、贡献注册表
  plugin-loader/  package manifest/export 校验、Composition、trust、受管安装
  plugin-sdk/     第三方插件稳定根入口
  builtins/       内建 Provider、工具、Session、CLI/TUI/RPC namespace entry
  coding-agent/   CLI 产品、默认 composition、MCP、会话、RPC
...
| 内置工具 | `packages/builtins/src/tool-*-implementation.ts` 与 namespace entry |
...
用 AI 辅助实现时，仍以可验证的仓库合同为准：先让模型定位 owning package、公开入口、既有测试与 README；将需求拆为一个可观察的失败行为；确认定向测试真的失败；再以最小修改修复。模型提出的 package 路径、CLI 参数、环境变量、JSON/RPC 字段和安全承诺都必须用源码或测试复核，不能把生成的解释当作证据。

在可插拔运行时中，先检查 entry 是否应属于 `builtins`、第三方 namespace package 或产品 bootstrap。不要让模型通过深导入、`any`、`eval`、shell 拼接、第二套 Agent loop 或“兼容旧 factory”来绕过边界。完成后按风险运行 workspace 测试、`npm run check`、build 或 release dry-run，并在同一变更中更新 owning README、插件指南和实施状态。
...
HTTP/SSE 传输复用同一 `RpcDispatcher`，入口是 `di-code-webui`，不是第二个 Agent loop。可运行的最小客户端示例见
[`examples/webui-client.ts`](../examples/webui-client.ts)；它使用 `DI_CODE_PROVIDER=faux`，并演示公开的 Session、附件、流事件、`OperationState`、取消和
`resumeToken`/`Last-Event-ID` 重连字段。示例不导入 `src` 或 `dist` 私有路径，也不绕过 token、Origin、workspace 或远程绑定限制。
...
自定义 TUI 通过公开 `@di-code/coding-agent/ui-host` 的 API v1 替换 `interactive-host`；fixture [`packages/coding-agent/test/fixtures/custom-tui.ts`](../packages/coding-agent/test/fixtures/custom-tui.ts)
同时使用 `@di-code/tui`，并由 `ui-host.test.ts` 验证取消、retry、Session 切换、渲染和生命周期失效。它不得访问 `SessionManager`、MCP manager、
command registry 或任何未声明 subpath。
