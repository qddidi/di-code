...
默认最大深度为 `2`、并发数为 `4`、单次超时为 `120000` ms、结果上限为 `32768` UTF-8 字节。`subagent` 可通过 `providerId` 选择插件 provider；结果超过上限会带 `[truncated]` 标记，异常和结果中的 `token`、`secret`、`authorization`、`password` 和 `api_key` 值会脱敏。取消、失败、超时和崩溃不会让父 Session 永久等待；它们分别形成 `cancelled` 或 `failed` 结果，并通过 frontend Controller 的受限 `subagents` 状态展示。RPC 输出的子 Agent 事件必须包含合法的 run、parent Session、status 和终态字段，否则客户端拒绝该记录。
