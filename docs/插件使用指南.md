...
完整 frontend 必须在 `start()` 中仅使用宿主传入的 `terminal`，并在用户退出时取消订阅、停止终端后才 resolve。`InteractiveController` 的 `state` 和事件包含流式消息、工具状态、队列、用量和诊断；输入只能通过它的 `submit()`、`steer()`、`cancel()`、命令、模型、Session 与压缩方法回到运行时。完整 frontend 必须声明并实现 `multiline-input`、`streaming`、`tool-status`、`cancel`、`retry`、`commands`、`model-selection`、`session-selection` 和 `compaction`；宿主在调用 `create()` 前校验这些核心能力，缺失时启动失败且不会回退到内置 UI。其他 `capabilities` 可用于可选降级。

Web 前端是独立的 Controller 投影，不共享 Node.js 插件对象。`WebFrontendHost` 通过 `WebTransport` 发送版本化 `hello`、`event`、`response` 和 `error` 消息；连接必须提供授权 token 与 frontend ID，slot action 还必须通过 slot 白名单。事件序号用于断线重连和过期事件拒绝，页面断开只解绑连接，不会取消或销毁宿主 Session。

浏览器侧可从 `@di-code/coding-agent` 使用 `createWebFrontendApp()`。它通过原生 WebSocket 创建 `WebSocketTransport`，渲染响应式 DOM 对话界面，并把输入、取消、重试、压缩和新 Session 动作提交给 `WebClient`。断线后把新 transport 传给 `app.reconnect()`；页面销毁只调用 `app.dispose()`，不能调用宿主的 `WebFrontendHost.dispose()`。短期 token、frontend ID 和服务端 `allowedActions` 必须由宿主显式配置，不能从页面信任的参数中推导。
...
已发布插件把运行时使用的 `@di-code/ai` 与 `@di-code/plugin-runtime` 写入自身 `dependencies`。
...
export default function projectStatusPlugin(api: PluginApi) {
  api.registerTool({
    ...
    execute: async (_toolCallId, _parameters, signal) => {
      ...
      return [{ type: "text", text: "Project status is available." }];
    }
  });
}
```
...
动态插件可能在 `session_start` 已发出后才通过 `plugin_run` 激活；宿主会在该 run 激活后向它重放一次当前 Session 的 `session_start`，因此动态 event handler 不会因激活时序而错过会话初始化。`session_shutdown` 发出后不会向之后激活的 run 重放过期的启动事件。
...
工具收到已校验参数和可选的取消信号；command handler 收到 `PluginCommandContext`；事件 handler 收到事件 payload 和可选的 `AbortSignal`。不要假定其中存在用户、Provider、会话历史、插件 manifest 或 UI 对象。

| 字段/方法 | 类型或结果 | 可用位置 | 用途和限制 |
| --- | --- | --- | --- |
| `ctx.cwd` | `string` | command | 当前工作根目录；读取项目文件前仍要自己限制目标路径。 |
| `ctx.signal` | `AbortSignal \| undefined` | command、工具、事件 | 监听取消；长任务应检查 `aborted` 并向下传递。 |
| `ctx.notify(message)` | `void` | command | 发送宿主管理的用户可见状态消息。 |
| `ctx.args` | `string` | command | slash command 名称之后的原始参数，插件必须自行解析和校验。 |
...
export default function projectStatusPlugin(api: PluginApi) {
  ...
  api.on("tool_execution_end", async (event, signal) => {
...
| `permissions.filesystem` | 是 | `none`、`read-project` 或 `read-write-project`。这是审计声明，不是 Node.js 沙箱。 |
...
interactive 模式显式启用 `--allow-dynamic-plugins` 并通过逐次确认后，动态插件在独立 Node 子进程中运行。子进程通过版本为 `1` 的 JSONL 记录注册或撤销 `tool`、`prompt`、`middleware` 和 `event` capability；tool 必须使用 `<plugin-id>__<tool-name>` 命名并提供 object JSON Schema。宿主会校验 package 声明的 capability、run/plugin 所有权、schema 和记录大小，重复注册、未声明能力、非法 namespace 或畸形记录会终止当前 run 并写入脱敏诊断。

能力注册后会进入下一轮 Agent 请求快照：tool 的 `execute`、prompt 的 `render`、middleware 的 `next()` 和 event handler 都通过带 `invokeId` 的双向 JSONL 调用执行。宿主只发送受限 `PluginContext`（`cwd`、模式、trust、模型）；取消时发送 `cancel`，子进程 handler 收到对应的 `AbortSignal`。工具结果必须是文本或图片内容块，宿主会检查结果形状并在 stop、失败、取消或 broker dispose 时撤销能力、拒绝挂起调用并回收子进程资源。子进程隔离生命周期和协议，但不是操作系统沙箱，动态代码仍按当前用户权限执行。

## 子 Agent

插件可以用 `registerSubagentProvider()` 提供受宿主约束的 provider。请求包含父 `Session`、工作目录、模型、可见工具/插件、深度、超时和结果字节上限；provider 只能返回 `SubagentRun`，不能自己维护另一套 Agent loop。内置 coding-agent 使用进程内 provider，并暴露 `subagent`、`subagent_list`、`send_message`、`wait`、`interrupt` 五个模型工具。

默认最大深度为 `2`、并发数为 `4`、单次超时为 `120000` ms、结果上限为 `32768` UTF-8 字节。`subagent` 可通过 `providerId` 选择插件 provider；`subagent_list` 只显示活动 run，`completed`、`failed` 和 `cancelled` run 会立即回收。近期终态结果会被有界缓存供 `wait` 读取；新活动 run 重用同一 ID 时会清除旧缓存，且 `wait` 与 `interrupt` 始终优先活动 run。重复 `interrupt` 已结束任务是安全的无操作。结果超过上限会带 `[truncated]` 标记，异常和结果中的 `token`、`secret`、`authorization`、`password` 和 `api_key` 值会脱敏。取消、失败、超时和崩溃不会让父 Session 永久等待；它们分别形成 `cancelled` 或 `failed` 结果，并通过 frontend Controller 的受限 `subagents` 状态展示。RPC 输出的子 Agent 事件必须包含合法的 run、parent Session、status 和终态字段，否则客户端拒绝该记录。

父 Session JSONL 使用现有 `version: 2`，新增 `type: "subagent"` 记录保存 run ID、start/end 状态和有上限的结果文本。该记录只用于恢复、树导航和 RPC/TUI 投影，不会自动进入模型上下文；未知、损坏或不兼容记录会产生 Session 诊断并停止追加。退出时宿主先取消当前 Session 的所有子 Agent，再释放插件 scope、MCP 和终端资源。
...
- `packages/plugin-runtime/src/index.ts`：插件公开类型、注册冲突、参数校验、handler 隔离和诊断。
