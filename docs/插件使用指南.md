...
`plugin list` 只显示 ID、启用状态和版本；`plugin get <id>` 输出不含安装路径和来源的公开状态。`enable` 与 `disable` 只更新受管 registry，不会 import 已禁用插件。运行 `di-code --trace-plugins` 或 `di-code --dump-composition` 可按需查看 resolved tree、Loader phase、owner Fiber、capability audit 和脱敏失败诊断；这两个开发观测 entries 不进入普通生产 composition，也不输出凭据、entry config 或完整请求体。
