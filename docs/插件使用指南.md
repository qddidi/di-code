...
| 模型工具、slash command、生命周期事件 | 热重载、插件市场、自定义 Provider、TUI 注入 |
| 项目本地插件和已安装的全局插件 | 用户登录态、用户级工具授权、服务端 Agent API |
| manifest 校验、入口路径保护、加载诊断 | 真正的文件、网络、进程权限沙箱 |
| `--trust-project` 决定是否 import 项目插件 | 用户自定义权限的运行时强制执行 |

`permissions` 是作者声明和审计信息，不会限制 Node.js 代码。只安装可信来源的插件；项目插件必须先由使用者显式信任。
...
模型是否调用工具由模型决定；工具名称清楚、描述具体、请求明确时最容易验证。插件没有加载时，先看终端 stderr 的 `plugin_diagnostic` 记录，后文有排查表。
...
项目插件只有在当前目录或祖先目录的 trust 决策为允许时才加载。`--trust-project` 保存此决策；它不会给插件额外 Node.js 权限。
...
| 现象 | 常见原因 | 先检查 |
| --- | --- | --- |
| 项目插件没有加载 | 项目未信任或目录错误 | 运行 `di-code --trust-project --interactive`；确认路径和 `plugin.json`。 |
...
- 项目插件仅在用户明确执行 `--trust-project` 后加载。
