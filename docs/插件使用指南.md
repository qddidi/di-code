...
interactive frontend 使用 `registerInteractiveFrontend()` 声明。frontend 由用户显式选择后成为当前 Session 唯一的终端所有者；它通过 `InteractiveController` 接收结构化事件并提交动作，不能自行调用 Agent、读取进程 stdio 或写 Session JSONL。内置 ANSI UI 的 ID 为 `builtin`，由 `@di-code/coding-agent` 的 `BuiltinInteractiveFrontend` 提供。
