...
RPC composition is split into `rpc-protocol-v1`, `rpc-server`, and `rpc-events`. `AgentSessionFactory` is a product-registered service, while `RpcMethodRegistry` owns the accepted method inventory. The v1 entry registers `prompt`, `cancel`, and `get_state` in the `di-code.rpc-v1` namespace; method additions must be registered before a server accepts them.
