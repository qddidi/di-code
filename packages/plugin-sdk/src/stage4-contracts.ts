...
export type SessionEventValidator = (payload: unknown) => boolean | undefined;
	...
	return {
		...
		migrate(event) {
			...
			const validate = (payload: SessionJsonValue): void => {
				...
				if (result === false) throw new Error(`Invalid payload for ${event.namespace}:${event.eventName}`);
			};
...
export interface SessionProjectionRegistry {
	...
	readonly replay: (events: readonly SessionEventEnvelope[]) => readonly SessionProjectionSnapshot[];
}
