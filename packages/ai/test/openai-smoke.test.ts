...
import { createOpenAIProvider, Type } from "../src/index.ts";

const enabled =
	process.env.DI_CODE_OPENAI_SMOKE === "1" &&
...
function smokeModel(): Model {
	return {
		id: process.env.OPENAI_MODEL?.trim() ?? "",
		name: "Configured OpenAI smoke model",
		provider: "openai",
...
describeSmoke("OpenAI Responses real smoke", () => {
	it("returns non-empty text", async () => {
		const provider = createOpenAIProvider({ models: [smokeModel()] });
