...
插件可在 `diCode.web` 声明 version `1` 的数据贡献。贡献只包含稳定 `slot`、`order`、`capability` 和宿主拥有的 `componentKey`，浏览器不会导入插件包或获得 command registry。只有当前运行时处于 active 的受管插件贡献会进入 Web manifest；启停操作在下次运行时重载后生效。未信任 Workspace 的项目插件 Web 声明不会进入 Web manifest，旧客户端可忽略未知 slot。
