...
export function GeneralSettings({ settings, theme, onThemeChange, onLocaleChange, onPermissionChange, onThinkingChange }: GeneralSettingsProps): React.JSX.Element {
	const { t } = useI18n();
	const currentProvider = settings.providers.find((provider) => provider.id === settings.runtime.providerId);
	const currentModel = currentProvider?.models.find((model) => model.id === settings.runtime.modelId);
	const reasoningEfforts = currentModel?.reasoningEfforts ?? [];
	const selectedThinking = settings.runtime.thinkingLevel && reasoningEfforts.includes(settings.runtime.thinkingLevel as (typeof reasoningEfforts)[number])
	...
	return <div className="settings-section" aria-labelledby="general-title">
		...
		<label className="setting-row"><span><strong>{t("Thinking")}</strong><small>{t("Current Session runtime level")}</small></span><select value={selectedThinking} onChange={(event) => onThinkingChange(event.target.value)}><option value="default">{t("Default")}</option>{reasoningEfforts.map((level) => <option value={level} key={level}>{t(level[0].toUpperCase() + level.slice(1))}</option>)}</select></label>
