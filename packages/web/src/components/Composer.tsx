import { Activity, Archive, ArrowLeft, ArrowUp, Check, ChevronDown, Command, CornerDownRight, FileClock, Gauge, Paperclip, RotateCcw, Settings2, Square, SlidersHorizontal, X } from "lucide-react";
...
interface ComposerProps {
	...
	readonly onTogglePlan?: () => Promise<void>;
}
...
export function Composer({ disabled = false, busy, attachments, imageInputSupported, onAddFiles, onRemoveAttachment, onSend, onSteer, onCancel, onCompact, onRunCommand, onRetry, onClear, onOpenSessions, onOpenTree, onOpenUsage, onOpenSettings, onLogout, onSetRuntime, onSetPermissionMode, onSetThinkingLevel, hero = false, modelLabel = "Model", activeRuntime, permissionMode, thinkingLevel, reasoningEfforts, retryable = false, runtimeOptions, usage, commands, restoredDraft, extensionPlaceholder, planMode, onTogglePlan }: ComposerProps): React.JSX.Element {
	...
	return <div className={`composer-wrap${hero ? " composer-hero" : ""}`}>
		<div ref={composer} className="composer" onDragOver={(event) => { if (imageInputSupported) event.preventDefault(); }} onDrop={(event) => { if (!imageInputSupported) return; event.preventDefault(); void onAddFiles(event.dataTransfer.files); }}>
			...
			<div className="composer-toolbar" aria-busy={busy}>
				<div className="composer-menu-anchor"><button className="composer-tool" type="button" aria-label={t("Commands")} title={t("Commands")} aria-expanded={menu === "commands"} aria-controls={menu === "commands" ? commandMenuId : undefined} onClick={() => { const opening = menu !== "commands"; setSelectedCommandIndex(0); setMenu(opening ? "commands" : undefined); if (opening) requestAnimationFrame(() => textarea.current?.focus()); }}><Command size={16} /></button>{menu === "commands" ? <div id={commandMenuId} className="composer-menu composer-command-menu" role="listbox" aria-label={t("Commands")}>{matchingCommands.length ? matchingCommands.map((command, index) => { const Icon = commandIcon(command.name); return <button id={`${commandMenuId}-${index}`} type="button" role="option" aria-selected={index === selectedCommandIndex} className={index === selectedCommandIndex ? "is-selected" : undefined} key={command.name} onMouseEnter={() => setSelectedCommandIndex(index)} onClick={() => selectCommand(command)}><Icon size={15} /><span className={`menu-item-copy${command.kind === "skill" ? " menu-item-skill" : ""}`}><strong>/{command.name}</strong><small>{command.description}</small></span></button>; }) : <p>{t("No matching commands")}</p>}</div> : null}</div>
				<button className="composer-tool" type="button" aria-label={imageInputSupported ? t("Add attachment") : t("Current model does not support image input")} title={imageInputSupported ? t("Add attachment") : t("Current model does not support image input")} onClick={() => input.current?.click()} disabled={disabled || !imageInputSupported || attachments.length >= 4}><Paperclip size={16} /></button>
				<div className="composer-menu-anchor"><button className="access-mode" type="button" aria-label={`${t("Permission mode")}: ${t(permissionModes.find((option) => option.id === permissionMode)?.label ?? "")}`} aria-expanded={menu === "access"} onClick={() => setMenu((value) => value === "access" ? undefined : "access")}><SlidersHorizontal size={14} />{t(permissionModes.find((option) => option.id === permissionMode)?.label ?? "")}<ChevronDown size={13} /></button>{menu === "access" ? <div className="composer-menu" role="menu" aria-label={t("Permission mode")}>{permissionModes.map((option) => <button type="button" role="menuitemradio" aria-checked={option.id === permissionMode} key={option.id} onClick={() => { setMenu(undefined); void onSetPermissionMode(option.id); }}><span>{t(option.label)}</span>{option.id === permissionMode ? <Check size={14} aria-label={t("Selected")} /> : null}</button>)}</div> : null}</div>
				{planMode?.active || planMode?.pending ? <div className={`composer-plan-meta${planMode.pending ? " is-pending" : ""}`} role="status"><span>{planMode.pending ? "Plan pending" : "Plan"}</span>{onTogglePlan ? <button type="button" aria-label="Turn off plan mode" title="Turn off plan mode" onClick={() => void onTogglePlan()} disabled={disabled || busy}><X size={13} /></button> : null}</div> : null}
				<span className="composer-spacer" />
				<div className="composer-menu-anchor">
					<button className="model-select" type="button" aria-label={`${t("Model")}: ${modelLabel}`} aria-expanded={menu === "runtime" || menu === "models" || menu === "reasoning"} onClick={() => setMenu((value) => value === "runtime" || value === "models" || value === "reasoning" ? undefined : "runtime")}><span>{modelLabel}</span><ChevronDown size={13} /></button>
					{menu === "runtime" ? <div className="composer-menu composer-menu-right" role="menu" aria-label={t("Model and reasoning")}><button type="button" role="menuitem" onClick={() => setMenu("models")}><span className="menu-item-copy"><small>{t("Model")}</small>{modelLabel}</span><ChevronDown size={14} /></button>{reasoningEfforts.length ? <button type="button" role="menuitem" onClick={() => setMenu("reasoning")}><span className="menu-item-copy"><small>{t("Reasoning")}</small>{t(thinkingLevel ?? "default")}</span><ChevronDown size={14} /></button> : null}</div> : null}
					{menu === "models" ? <div className="composer-menu composer-menu-right composer-model-menu" role="menu" aria-label={t("Models")}><button type="button" className="menu-back" role="menuitem" onClick={() => setMenu("runtime")}><ArrowLeft size={14} />{t("Model and reasoning")}</button>{Array.from(new Map(runtimeOptions.map((option) => [option.providerId, option.providerName])).entries()).map(([providerId, providerName]) => <div className="model-group" key={providerId}><p>{providerName}</p>{runtimeOptions.filter((option) => option.providerId === providerId).map((option) => { const selected = option.providerId === activeRuntime.providerId && option.modelId === activeRuntime.modelId; return <button type="button" role="menuitemradio" aria-checked={selected} key={`${option.providerId}/${option.modelId}`} onClick={() => { setMenu(undefined); void onSetRuntime(option.providerId, option.modelId); }}><span>{option.label}</span>{selected ? <Check size={14} aria-label={t("Selected")} /> : null}</button>; })}</div>)}</div> : null}
