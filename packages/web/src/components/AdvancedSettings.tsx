...
function PresetsPanel({ settings, onError, onSuccess, onSettingsChange, sessionId }: Pick<AdvancedSettingsProps, "settings" | "onError" | "onSuccess" | "onSettingsChange" | "sessionId">): React.JSX.Element {
	const { t } = useI18n();
	...
	const save = (): void => { if (!name.trim()) return; const next = [...presets, { id: crypto.randomUUID(), name: name.trim(), providerId: settings.runtime.providerId, modelId: settings.runtime.modelId, thinkingLevel: settings.runtime.thinkingLevel, permissionMode: settings.permissionMode }]; setPresets(next); localStorage.setItem("di-code-presets", JSON.stringify(next)); setName(""); };
	const apply = async (preset: Preset): Promise<void> => { try { if (preset.providerId && preset.modelId) await callRpc("set_runtime", { sessionId, providerId: preset.providerId, modelId: preset.modelId }); if (preset.permissionMode) await callRpc("set_permission_mode", { permissionMode: preset.permissionMode }); if (preset.thinkingLevel) await callRpc("set_thinking_level", { sessionId, level: preset.thinkingLevel }); await onSettingsChange(); onSuccess(t("Preset applied.")); } catch (cause) { onError(cause instanceof Error ? cause.message : t("Preset apply failed.")); } };
...
export function AdvancedSettings({ tab, settings, onError, onSuccess, onSettingsChange, sessionId }: AdvancedSettingsProps): React.JSX.Element {
	switch (tab) {
		...
		case "presets": return <PresetsPanel settings={settings} onError={onError} onSuccess={onSuccess} onSettingsChange={onSettingsChange} sessionId={sessionId} />;
