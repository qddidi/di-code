		...
		return <>
			...
			{visible.map((session) => { const selected = session.id === activeSessionId; const running = runningSessionIds?.has(session.id) ?? false; return <div className={`session-item-wrap${selected ? " is-active" : ""}`} key={session.id}>
				...
				{running || actionsEnabled ? <div className={`session-item-actions${running ? " is-running" : ""}`}>
					...
					{actionsEnabled && !running ? <><button className="session-more" type="button" aria-label={`Actions for ${session.label || "Untitled session"}`} aria-expanded={menu === session.id} onClick={(event) => { event.stopPropagation(); setMenu((current) => current === session.id ? undefined : session.id); }}><MoreHorizontal size={15} /></button>{menu === session.id ? <div className="session-action-menu" role="menu"><button type="button" role="menuitem" onClick={() => { setMenu(undefined); setEditing(session.id); setLabel(session.label); }}><Pencil size={13} />Rename</button><button type="button" role="menuitem" onClick={() => { setMenu(undefined); void onBranch(session.id); }}><GitBranch size={13} />Branch</button><button type="button" role="menuitem" onClick={() => { setMenu(undefined); void onInspect(session.id).then(setInspect); }}>i <span>Inspect</span></button><button type="button" role="menuitem" onClick={() => { setMenu(undefined); if (window.confirm("Delete this session?")) void onDelete(session.id); }}><Trash2 size={13} />Delete</button></div> : null}</> : null}
				</div> : null}
			</div>; })}
		</div>
