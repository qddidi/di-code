...
export function Trajectory({ tools, contextFiles, compaction }: { readonly tools: readonly ToolTrace[]; readonly contextFiles: readonly ContextFile[]; readonly compaction?: { readonly state: string; readonly reason: string; readonly error?: string } }): React.JSX.Element {
	const { t } = useI18n();
	return <section className="trajectory" aria-label={t("Trajectory")}>
		...
		{contextFiles.length ? <details className="trajectory-context"><summary><FileCode2 size={15} />{t("Context files")} ({contextFiles.length})</summary><ul>{contextFiles.map((file) => <li key={file.path}><code>{file.path}</code><span>{file.scope}</span></li>)}</ul></details> : null}
