...
import { PromptQueue } from "./components/PromptQueue.tsx";
...
function App(): React.JSX.Element {
	...
	useEffect(() => {
		const element = mainContentRef.current;
		const sessionChanged = previousSessionRef.current !== conversation.activeSessionId;
		const tabChanged = previousTabRef.current !== tab;
		const loadingFinished = previousLoadingRef.current && !loading;
		const responseFinished = previousBusyRef.current && !busy;
		if (element && !loading && (sessionChanged || tabChanged || loadingFinished || responseFinished)) scrollToBottom();
		previousBusyRef.current = busy;
		previousSessionRef.current = conversation.activeSessionId;
		previousTabRef.current = tab;
		previousLoadingRef.current = loading;
	}, [busy, conversation.activeSessionId, conversation.queuedPrompts.length, conversation.steeringPrompts.length, loading, scrollToBottom, tab]);
