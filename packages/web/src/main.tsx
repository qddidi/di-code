...
function App(): React.JSX.Element {
	...
	return <div className={`app-shell${sidebarCollapsed ? " sidebar-collapsed" : ""}${hero ? " is-hero" : " is-active"}`}>
		...
		<div className="main-column">{!hero ? <WorkspaceBar title={activeTitle} onToggleSidebar={() => setSidebarCollapsed((value) => !value)} onSettings={() => setSettingsOpen(true)} onSessionLog={() => setSessionLogOpen(true)} /> : null}<main ref={mainContentRef} className={`main-content${hero ? " is-hero" : ""}`}>
			...
			{(() => {
				const state = conversation.projections["plan:mode"]?.state;
				if (!state || typeof state !== "object" || state === null) return null;
				const plan = state as { readonly active?: unknown; readonly pending?: unknown };
				if (plan.active !== true && plan.pending !== true) return null;
				return <div className="plan-mode-banner" role="status">{plan.pending === true ? "Plan mode change pending" : "Plan mode active"}</div>;
			})()}
