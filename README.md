...
交互 TUI 启动时，如果当前工作区存在尚未信任的项目插件、Skills 或 MCP 配置，会先询问是否信任；随后会显示插件、Skills、MCP 及成功/失败诊断摘要。WebUI 首次进入工作区也会用弹窗确认是否加载这些项目资源，设置页会列出托管插件和项目插件及其状态。详见[插件使用指南](docs/插件使用指南.md)。
